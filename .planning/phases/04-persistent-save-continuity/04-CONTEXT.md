...
- **D-40:** **Escalate on undecidability, never on duration or count.** Offline queues, slow uploads, and old local-only versions never escalate — escalating them means escalating *success*. Locked user-facing nouns: **save**, **version**, **Save history**, **your server**. Banned from all player-facing copy: *conflict, revision, sync, backed up, uploaded, digest, cursor, journal, parent, head, merge*. Locked state labels: `Only on this Mac` / `Waiting to copy` / `On your server` / `You are here` / `Restored here {date}` / `Separate version`, each with the full VoiceOver sentence recorded in `discussion-research/E-save-state-surfacing.md`. The only-copy destructive modal is locked verbatim: **"This is the only copy of your progress."** — *"{N} versions of {title} are only on this Mac and nowhere else. Continuing removes them for good."* → **Export saves…** (default) · Cancel · Remove anyway.
...
- **D-54:** **"Conflict" never appears in player-facing copy.** The feature is named **"Two versions of your progress."** Locked strings including the inbox item, sheet, side lines, buttons, clock caveat, not-downloaded case, post-resolution confirmations, and the two console-only variants are recorded verbatim in `discussion-research/G-conflict-resolution-ux.md` and are locked here by reference. Banned from player copy: *conflict, overwrite, lost, wins, loser, merge, error, failed, corrupt, stale, invalid*. **[Arbitration]** Where area E's rollup said "Two versions of this save", G's noun wins for headings and rollups; E's `Separate version` survives as the per-row timeline label.
