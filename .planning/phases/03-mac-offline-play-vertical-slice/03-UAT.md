...
[testing paused — 5 items outstanding: 2 blocked (items 8, 9 — physical controller hardware) and 3 partial (item 7's quit-and-return and refusal-and-exit clauses -- its offline clause closed 2026-09-13, see below, item 10's physical-controller/experiential-VoiceOver half, item 13's real-BIOS-bytes acceptance). Item 7 moved from fully blocked to partial on 2026-09-12: the owner downloaded and played a real game with the real pinned mGBA on their own machine — the first real launch this project has had, every prior record having used `/bin/echo` for the emulator — but three clauses of that item (network disabled, quit-and-return, digest-mismatch refusal plus exit classification) were not exercised and were recorded as separate blocked sub-records rather than rounded up. The network-disabled clause was then closed for real on 2026-09-13: Advance Wars launched and ran on the pinned mGBA with `caddy` stopped thirteen minutes beforehand. `playstead-mac/docs/UAT-07-RUNBOOK.md` is the procedure for the two clauses still open. Items 45-49 were merged from plans 03-11/03-13/03-15, whose human checkpoints had never reached this file; 45, 46, 47, 48 and 49 are all closed, and 48 turned out to be a real defect (see its evidence). The `## Summary` footer below IS reconciled against these per-item results by `scripts/check-uat-tally.sh`; this annotation is hand-maintained and should be read as such.]
