defmodule PlaysteadWeb.DevicesLive.DeviceRow do
  ...
  def device_row(assigns) do
    ...
    >
      <div class="min-w-0">
        <form :if={@renaming_id == @device.id} phx-submit="rename" class="flex items-center gap-2">
          ...
          />
          <button type="submit" class="text-sm font-semibold text-[#38BDF8]">Save</button>
          <button type="button" phx-click="cancel_rename" class="text-sm text-[#94A3B8]">
            Cancel
          </button>
        </form>
        ...
        <p :if={@renaming_id != @device.id} class="flex items-center gap-2">
          ...
          <button
            ...
            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-md text-[#94A3B8] hover:bg-[#334155]"
          >
        ...
        <p :if={@fingerprint} class="mt-1 font-['JetBrains_Mono'] text-[14px] text-[#94A3B8]">
          {@fingerprint}
        </p>
        <p :if={@tombstone} class="mt-1 text-sm text-[#94A3B8]">
