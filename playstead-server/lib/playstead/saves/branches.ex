defmodule Playstead.Saves.Branches do
  ...
  makes "no automatic winner, ever" true by construction rather than
  by convention.

  A child reference comes from either `Revision.parent_revision_id`
  (the ordinary single-parent case) or a `RevisionParent` row (a
  resolution revision naming more than one parent, D-48) -- a head is
  a revision named as a parent by neither.
