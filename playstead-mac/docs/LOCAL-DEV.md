...
| **Play seems to launch but nothing happens, and the app never notices an exit** | Almost certainly Gatekeeper holding the emulator. A quarantined bundle is left suspended before it ever runs, and `AdapterHost.launch` cannot tell that apart from a running emulator — so the app waits forever with no error. Fix: open the installed emulator once by hand in Finder (right-click → **Open** → **Open**) to clear quarantine, then press Play again. You can confirm the hang with `ps -o stat=,command= -p $(pgrep -n mGBA)` and `xattr -p com.apple.quarantine <path to mGBA.app>`. Tracked as broken window #8. |
